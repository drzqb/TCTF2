...
from tensorflow.keras.layers import Input, Layer, Dense, Dropout, Conv2D, Embedding
...
params_check = "modelfiles/textclassify_eb_bce"
params_hidden_size = 100
params_label_size = 2
params_batch_size = 10
params_num_filters = [64, 64, 64, 64]
...
class TextCNN(Layer):
    ...
    def build(self, input_shape):
        self.convs = []
        for i in range(len(params_num_filters)):
            self.convs.append(
                Conv2D(filters=params_num_filters[i], kernel_size=[params_kernel_sizes[i], params_hidden_size],
                       padding="VALID", activation="relu"))
        self.dropout = [Dropout(params_drop_rate) for _ in range(len(params_num_filters) + 1)]
    ...
    def call(self, inputs, **kwargs):
        ...
        for i in range(len(params_num_filters)):
            ...
            convs.append(pool)
        concat = tf.concat(convs, axis=2)
        flatten = tf.reshape(concat, [-1, np.sum(params_num_filters)])
        dropout = self.dropout[-1](flatten)
...
class TextClassify():
    ...
    def build_model(self, summary=True):
        ...
        dropout = TextCNN(name="textcnn")(now)

        logits = Project()(dropout)
