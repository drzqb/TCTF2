'''
    基于 BiGRU + attention 的评价二分类
    基于词
'''
import tensorflow as tf
from tensorflow.keras.models import Model
from tensorflow.keras.layers import Input, Layer, Dense, Dropout, Embedding, Bidirectional, GRU
...
params_check = "modelfiles/textclassify_eb_GRU_attn_flw"
...
class Embeddings(Layer):
    ...
    def call(self, sen, **kwargs):
        ...
        attn_mask = tf.greater(sen, 0)
        return sen_embed, attn_mask
...
class Attention(Layer):
    ...
    def call(self, inputs, **kwargs):
        x, mask = inputs  # x: B*N*H mask: B*N

        r = tf.squeeze(tf.matmul(x, self.w), axis=2)
        score = tf.expand_dims(softmax(r, mask), axis=1)

        return self.dropoutattn(tf.squeeze(tf.matmul(score, x), axis=1))
...
class TextClassify():
    ...
    def build_model(self, summary=True):
        ...
        now, attn_mask = Embeddings(name="embeddings")(sen)

        now = Bidirectional(GRU(params_rnn_size,
                                return_sequences=True,
                                ), name="bigru")(now)

        attn = Attention(name="attn")(inputs=(now, attn_mask))
    ...
    def train(self, train_file, val_file):
        ...
        # sigmoid crossentropy loss with sample_weight
        # lossobj = bce_loss_weight
