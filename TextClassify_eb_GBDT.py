...
def train(data):
    ...
    print('max_depth=3 precision_score:', clf.score(x_test, y_test))

    clf = GradientBoostingClassifier(
        max_depth=10,
    )
    clf.fit(x_train, y_train)
    print('max_depth=10 precision_score:', clf.score(x_test, y_test))

    clf = GradientBoostingClassifier(
        max_depth=20,
    )
    clf.fit(x_train, y_train)
    print('max_depth=20 precision_score:', clf.score(x_test, y_test))

    clf = GradientBoostingClassifier(
        max_depth=3,
    )
    clf.fit(x_train, y_train, sample_weight)
    print('\nmax_depth=3,sample_weight precision_score:', clf.score(x_test, y_test))

    clf = GradientBoostingClassifier(
        max_depth=10,
    )
    clf.fit(x_train, y_train, sample_weight)
    print('max_depth=10,sample_weight precision_score:', clf.score(x_test, y_test))

    clf = GradientBoostingClassifier(
        max_depth=20,
    )
    clf.fit(x_train, y_train, sample_weight)
    print('max_depth=20,sample_weight precision_score:',
          clf.score(x_test, y_test))  # 第一个sample_weight有下降，第二个sample_weight有提升
